...
Today i learnt about a problem from plaidctf zipper  from  the writeup from bl4de's repo :)
...
Filename length (in bytes) shows that it should be 9001 bytes length, which is insane. Let's try to cut this off into reasonable 8 bytes:
...
We get CENTRAL HEADER back, now we have to fix its Filename Length field as well. As we can see, the filename itself is blank, so we fill it using PlaidCTF string, which is exactly 8 bytes length. How nice :)
